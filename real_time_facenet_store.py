...
# Create a text file to store the recognized data
file = open('recognized_data.txt', 'w')
...
while True:
    ...
    for face in faces:
        ...
        if min_distance < threshold:
            ...
            # Format the data with proper alignment
            data = f"Name: {recognized_name:<20} Timestamp: {timestamp}\n"

            # Write the formatted data to the text file
            file.write(data)
            file.flush()

            print(f"Recognized: {recognized_name} ({timestamp})")
